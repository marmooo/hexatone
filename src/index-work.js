...
function handlePointerDown(event, panel, groupId) {
  ...
  if (selectedHits.length === 2) {
    state.initialOrientation = getHitOrientation(
      selectedHits[0],
      selectedHits[1],
    );
    state.basePadHits = [selectedHits[0], selectedHits[1]];
    state.currentPadHit = selectedHits[0];
    state.targetPadHit = selectedHits[1];
    const expression = calcExpressionFromMovement(event, state);
    if (expression !== null) {
      state.chordExpression = expression;
    } else {
  ...
  selectedHits.forEach((padHit) => mpePointerDown(event, padHit, groupId));
  mpeHitMap.set(event.pointerId, new Set(selectedHits));
}
...
function initGroup(group, groupId, allKeys, gapPercent, radius) {
  ...
  svg.querySelectorAll(".pad-hit").forEach((padHit) => {
...
function releaseChannel(channelNumber) {
  ...
  midy.setControlChange(channelNumber, 11, 64);
